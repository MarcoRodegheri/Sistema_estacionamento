...
public class PopupSecundario extends Dialog {
    ...
    public PopupSecundario(Cliente clienteParaEditar, FinalizarListener listener) {
        ...
        if (clienteParaEditar instanceof Estudante)
            this.tipoUsuario = "Estudante";
        else if (clienteParaEditar instanceof Tecnopuc)
            this.tipoUsuario = "Tecno Puc";
        else
    ...
    private void montarLayout() {
        ...
        if (clienteEmEdicao == null && voltarListener != null) {
            Button btnVoltar = new Button("Voltar", e -> {
                close();
                voltarListener.run();
            });
    ...
    private void salvar() {
        try {
            if (txtNome.getValue().isEmpty() || txtCpf.getValue().isEmpty()) {
                Notification.show("Preencha nome e CPF!");
                return;
            }
    ...
    private void estilizarBotaoFinalizar(Button btn) {
        btn.getStyle().set("background", "#27AE60").set("color", "white").set("width", "100%").set("margin-top",
                "10px");
    }

...
public class MainView extends VerticalLayout {
    ...
    public MainView() {
        ...
        HorizontalLayout layoutBotaoUtil = new HorizontalLayout(btnTotalPorData, btnReceitaTotal, btnGerarRelatorio, btnOutrasConsultas);
        ...
        card.setWidth("98%"); card.setMaxWidth("1600px");
        ...
        listas.setWidthFull(); listas.setSpacing(true);
        ...
        gridClientes.getStyle().set("font-size", "12px"); gridClientes.setHeight("300px"); 
        
        gridClientes.addItemClickListener(event -> { if (event.getItem() != null) abrirPopupEdicao(event.getItem()); });
        ...
        gridVeiculos.addColumn(u -> {
            return ger.getTodosClientes().stream().filter(c -> c.possuiVeiculo(u.getPlaca())).findFirst().orElse(null).getNome();
        }).setHeader("Cliente").setAutoWidth(true);

        gridVeiculos.addColumn(UsoDeVaga::getPlaca).setHeader("Placa");
        gridVeiculos.addColumn(u -> u.getEntrada().format(DateTimeFormatter.ofPattern("dd/MM HH:mm"))).setHeader("Chegada");
        
        gridVeiculos.addThemeVariants(GridVariant.LUMO_ROW_STRIPES, GridVariant.LUMO_COMPACT);
        gridVeiculos.getStyle().set("font-size", "12px"); gridVeiculos.setHeight("300px"); 
        c2.add(t2, gridVeiculos);

        listas.add(c1, c2);
        c1.setWidth("60%"); c2.setWidth("40%");
    ...
    private void abrirPopupPrincipal() { new PopupPrincipal(this::abrirPopupSecundario).open(); }
    private void abrirPopupSecundario(String t) { new PopupSecundario(t, this::salvarCliente, this::abrirPopupPrincipal).open(); }
    private void abrirPopupEdicao(Cliente c) { new PopupSecundario(c, this::salvarCliente).open(); }
    private void abrirTotalEntradas() { new TotalEntradas().open(); }
    private void abrirReceitaTotal() { new ReceitaTotal().open(); }
    private void abrirGerarRelatorio() { new GerarRelatorio().open(); }
    private void abrirOutrasConsultas() { new OutrasConsultas().open(); }
    private void abrirFinanceiro(Cliente cliente) {
        ...
        if (cliente instanceof Estudante) {
            ...
            com.vaadin.flow.component.combobox.ComboBox<Double> combo = new com.vaadin.flow.component.combobox.ComboBox<>("Recarga");
            ...
            com.vaadin.flow.component.combobox.ComboBox<Integer> comboMes = new com.vaadin.flow.component.combobox.ComboBox<>("Mês de Referência");
            ...
            Button btnPagar = new Button("Pagar Boleto Selecionado", ev -> {
                if (comboMes.getValue() != null) {
                    ...
                    Notification.show("Boleto do mês " + comboMes.getValue() + " pago com sucesso!", 3000, Notification.Position.BOTTOM_START)
                                .addThemeVariants(com.vaadin.flow.component.notification.NotificationVariant.LUMO_SUCCESS);
                    janela.close();
                }
            });
    ...
    private void salvarCliente(Cliente c) {
        try {
            ...
            if (!jaExiste) ger.cadastrarCliente(c);
            Notification.show("Salvo: " + c.getNome(), 3000, Notification.Position.BOTTOM_START);
            atualizarLogs();
        } catch (Exception e) {
            Notification.show("Erro: " + e.getMessage(), 3000, Notification.Position.BOTTOM_START).addThemeVariants(com.vaadin.flow.component.notification.NotificationVariant.LUMO_ERROR);
        }
    }
    ...
    private void abrirPesquisa() {
        ...
        TextField tPlaca = new TextField("Placa do Veículo"); tPlaca.setPlaceholder("XXX-0000"); tPlaca.setAutofocus(true); tPlaca.setWidthFull();
        com.vaadin.flow.component.datetimepicker.DateTimePicker pickerEntrada = new com.vaadin.flow.component.datetimepicker.DateTimePicker("Data/Hora Entrada");
        pickerEntrada.setValue(LocalDateTime.now().minusHours(1)); pickerEntrada.setWidthFull();
        com.vaadin.flow.component.datetimepicker.DateTimePicker pickerSaida = new com.vaadin.flow.component.datetimepicker.DateTimePicker("Data/Hora Saída (Apenas p/ Saída)");
        pickerSaida.setValue(LocalDateTime.now()); pickerSaida.setWidthFull();
        ...
        Button bEntrada = new Button("Registrar Entrada", e -> {
            try {
                ...
                Cliente c = ger.getTodosClientes().stream().filter(cli -> cli.possuiVeiculo(placa)).findFirst().orElse(null);
                ...
                if (isAviso) notif.addThemeVariants(com.vaadin.flow.component.notification.NotificationVariant.LUMO_ERROR);
                else notif.addThemeVariants(com.vaadin.flow.component.notification.NotificationVariant.LUMO_SUCCESS);
        ...
        bEntrada.getThemeNames().add("primary success"); bEntrada.setWidthFull();

        Button bSaida = new Button("Registrar Saída", e -> {
            try {
                String placa = tPlaca.getValue().trim().toUpperCase();
                Cliente cliente = ger.getTodosClientes().stream().filter(c -> c.possuiVeiculo(placa)).findFirst().orElse(null);
                ...
                if (valorMostrado > 0) notif.addThemeVariants(com.vaadin.flow.component.notification.NotificationVariant.LUMO_SUCCESS);
                else notif.addThemeVariants(com.vaadin.flow.component.notification.NotificationVariant.LUMO_PRIMARY);
        ...
        bSaida.getThemeNames().add("primary error"); bSaida.setWidthFull();
    ...
    private void estilizarBotaoGrande(Button btn) {
        btn.getStyle().set("background", "#001ba6").set("color", "white").set("padding", "15px 25px")
           .set("font-size", "14px").set("font-weight", "bold").set("border-radius", "8px").set("cursor", "pointer");
        btn.setWidth("350px");
    }
